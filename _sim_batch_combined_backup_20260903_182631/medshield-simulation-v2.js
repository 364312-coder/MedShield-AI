(function(){
  ...
  const sim={mode:"single",selected:["IMAGE_LEAKAGE"],cases:[],activeId:null,logs:[],started:false,sequence:0,timelines:[],dialogQueue:[],dialogQueueActive:false};
  ...
  function replayAll(){
    ...
    cancelTimelines();sim.dialogQueue=[];sim.dialogQueueActive=false;closeDialog();sim.started=false;sim.logs=[];sim.cases=[];sim.activeId=null;
  ...
  async function autoProcessAll(){
    ...
    render();
    /* Multi-event execution keeps evidence separate: one dialog per completed event. */
    const completed=cases.filter(c=>c.phase==="COMPLETE");
    if(completed.length){
      sim.dialogQueue=completed.map(c=>c.id);
      sim.dialogQueueActive=true;
      setTimeout(showNextQueuedDialog,100);
    }

  }

  function reset(){cancelTimelines();sim.started=false;sim.cases=[];sim.activeId=null;sim.logs=[];sim.dialogQueue=[];sim.dialogQueueActive=false;render();}
  ...
  function closeDialog(){
    ...
    if(sim.dialogQueueActive)setTimeout(showNextQueuedDialog,120);
  }
