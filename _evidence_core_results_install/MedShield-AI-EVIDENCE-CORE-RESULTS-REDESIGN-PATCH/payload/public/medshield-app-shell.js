(function(){
  ...
  function moveEvidence(view){
    ...
    const modelPanel=q("#model-metrics")?.closest(".metric-panel");
    ...
    /* Core results is presentation-first: one detection column + one security-outcome column.
       Values below are read-only formal FULL-run evidence; no browser-side recomputation. */
    if(metrics){
      metrics.classList.add("app-core-results-section");
      const head=q(".section-head",metrics);
      if(head) head.innerHTML='<span>FORMAL FULL RUN</span><h2>正式运行证据</h2><p>正式 FULL run 结果，仅展示检测、防御与恢复证据；前端不重新训练，也不重算策略。</p>';
      const layout=q(".metrics-layout",metrics);
      if(layout){
        layout.classList.add("app-core-results-grid");
        if(modelPanel){
          modelPanel.classList.add("app-core-detection");
          modelPanel.innerHTML=`
            <div class="app-core-panel-head"><span>AI DETECTION</span><h3>AI 检测表现</h3></div>
            <div class="app-core-metric-rows">
              <div><span>Accuracy</span><strong>98.44%</strong><i style="--v:.9844"></i></div>
              <div><span>Precision</span><strong>98.66%</strong><i style="--v:.9866"></i></div>
              <div><span>Recall</span><strong>96.10%</strong><i style="--v:.961"></i></div>
              <div><span>F1</span><strong>97.36%</strong><i style="--v:.9736"></i></div>
              <div><span>AUC</span><strong>98.61%</strong><i style="--v:.9861"></i></div>
              <div><span>False Positive Rate</span><strong>0.56%</strong><i class="low" style="--v:.056"></i></div>
            </div>`;
        }
        const summary=document.createElement("aside");
        summary.className="app-core-summary";
        summary.innerHTML=`
          <div class="app-core-conclusion">
            <span>FORMAL RESULT</span>
            <p>50,000 条正式事件中，系统取得 98.44% Accuracy，并实现 100% 未见攻击检出率与 96.10% 攻击遏制率。</p>
          </div>
          <div class="app-core-primary"><span>总体 Accuracy</span><strong>98.44%</strong><small>FULL RUN · 50,000 EVENTS</small></div>
          <div class="app-core-outcomes">
            <div><span>攻击遏制率</span><strong>96.10%</strong><small>14,415 / 15,000</small></div>
            <div><span>未见攻击检出率</span><strong>100%</strong><small>3,000 unseen attacks</small></div>
          </div>
          ...
          <div class="app-core-evidence-strip">
            ...
            <div><span>可信恢复</span><strong>2 / 2 成功</strong></div>
        ...
        next.type="button";
        next.className="app-core-next";
        next.dataset.evidenceTab="scenarios";
        next.innerHTML='<span>下一步</span><strong>查看不同攻击场景的检测表现</strong><i>→</i>';
        metrics.appendChild(next);
      }
    }
