(function(){
  ...
  function buildOverview(view){
    ...
    stage.innerHTML=`
      <div class="app-overview-flow" aria-label="MedShield-AI 核心闭环">
        ...
        <div class="app-overview-node"><span>风险研判</span><strong>U/F/N → AutoEncoder → LightGBM</strong></div><div class="app-overview-arrow">→</div>
      ...
      <div class="app-overview-foot">
        <article><span>BASELINE PROTECTION · B(D)</span><h3>常驻保护不是事后补救。</h3><p>AES-256-GCM、权限、密钥管理、水印/内容身份与分级备份共同构成 B(D) 常驻安全底线；动态策略只能继续收紧，不能降低原有保护。</p></article>
  ...
  function moveRisk(view){
    view.appendChild(pageHead(
      "02 · RISK ASSESSMENT",
      "风险研判，不把 AI 做成黑盒。",
      "用户、文件与网络行为形成 U / F / N，经统一为 X 后由 AutoEncoder 计算异常偏离 E_AE，再由 LightGBM 融合形成 R 与 A；AI 只负责风险感知，最终防御由 Policy Engine 决定。"
    ));
  ...
  function moveCapabilities(view,recovery){
    view.appendChild(pageHead(
      ...
      "从预防、响应，到可信恢复。",
      "这一视图只解释发现风险之后系统如何保护医疗数据：AI 输出 R/A，Policy Engine 再结合 D、C 与 Trap 生成 P0–P3；常驻保护始终存在，完整性异常时才进入可信恢复。"
    ));
  ...
  function moveEvidence(view){
    ...
    const scenarios=evidenceSection("攻击场景表现","SCENARIO PERFORMANCE","按正式攻击场景查看异常检出表现；这些结果验证风险感知层在不同场景下的表现，场景检出率不等同于异常类型分类 Recall。");
    if(scenarioPanel) moveExisting(scenarioPanel,q(".metrics-layout",scenarios));

    const defense=evidenceSection("防御、完整性与恢复","DEFENSE & RECOVERY","集中查看策略、防御处置、联合完整性与条件恢复结果，用于验证检测之后的安全闭环是否有效。");
    ...
    if(metricsLayout){
      ...
      summary.innerHTML=`
        <div class="app-core-conclusion"><span>FORMAL FULL RUN</span><strong>98.44%</strong><b>总体 Accuracy</b><p>在 50,000 条正式事件中完成检测、处置、完整性与恢复链路验证。</p></div>
        <div class="app-core-outcomes">
          <div><strong>96.10%</strong><span>攻击遏制率</span></div>
          <div><strong>100%</strong><span>本次正式 run 未见攻击检出率</span></div>
        </div>
        <div class="app-core-scale" aria-label="正式实验规模">
          ...
          <div><strong>3,000</strong><span>本次正式 run 未见攻击事件</span></div>
  ...
  function setEvidenceTab(key,focus=true){
    ...
    if(focus && app.activeView==="evidence"){
      ...
      if(active) requestAnimationFrame(()=>active.scrollIntoView({block:"start",inline:"nearest",behavior:"auto"}));
    }
  }
