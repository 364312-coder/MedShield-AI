...
try {
  ...
  $index = [System.Text.RegularExpressions.Regex]::Replace($index,'medshield-app-shell\.css\?v=[^"''\s>]+','medshield-app-shell.css?v=evidence-proof-v5-20260903')
  $index = [System.Text.RegularExpressions.Regex]::Replace($index,'medshield-app-shell\.js\?v=[^"''\s>]+','medshield-app-shell.js?v=evidence-proof-v5-20260903')
  Write-Utf8NoBom $indexPath $index

  # Structural validation.
  $checkJs = [System.IO.File]::ReadAllText($jsPath)
  if (($checkJs.Split('function moveEvidence(view){').Count - 1) -ne 1) { throw "STOP: moveEvidence count validation failed." }
  ...
  Write-Host "" 
  Write-Host "MedShield-AI Evidence Proof V5 installed and validated." -ForegroundColor Green
