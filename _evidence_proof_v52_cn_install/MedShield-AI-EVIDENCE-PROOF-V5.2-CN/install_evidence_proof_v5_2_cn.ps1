...
try {
  ...
  if ($jsNew -notmatch 'EVIDENCE_PROOF_V5_2_CN' -or $jsNew -notmatch 'evidence-proof-dashboard') {
  ...
  $index = [System.Text.RegularExpressions.Regex]::Replace($index,'medshield-app-shell\.css\?v=[^"''\s>]+','medshield-app-shell.css?v=evidence-proof-v5-2-cn-20260903')
  $index = [System.Text.RegularExpressions.Regex]::Replace($index,'medshield-app-shell\.js\?v=[^"''\s>]+','medshield-app-shell.js?v=evidence-proof-v5-2-cn-20260903')
  ...
  if ($checkJs -notmatch '攻击场景检出表现' -or $checkJs -notmatch '安全事件确认与反馈') { throw "STOP: new Evidence DOM validation failed." }
  ...
  Write-Host "" 
  Write-Host "MedShield-AI Evidence Proof V5.2 CN installed and validated." -ForegroundColor Green
