(function(){
  ...
  const VERSION = "V2-CORE-GUIDED-RESULTS";
  ...
  function renderLog(){
    ...
    const html=sim.logs.length?sim.logs.map(i=>`<div class="sim-stream-line ${esc(i.tone)}" data-open-stream><time>${esc(i.time)}</time><b>${esc(i.type)}</b><span>${esc(i.message)}</span></div>`).join(""):'<div class="sim-stream-line"><time>--:--:--</time><b>系统</b><span>等待实验启动。</span></div>';
  ...
  function renderPhaseRail(){
    ...
    host.innerHTML=PHASES.map(([_,label],i)=>{
      ...
      return `<div class="sim-phase-step ${current?"current":""} ${done?"done":""}"><i>${done?"✓":String(i+1).padStart(2,"0")}</i><span>${label}</span></div>`;
    }).join("");
  }
  ...
  function renderFlow(){
    ...
    if(lanes)lanes.innerHTML=sim.cases.map(item=>`<span class="sim-parallel-lane ${item.id===sim.activeId?"current":""} ${item.phase==="COMPLETE"?"done":""}">${esc(scenarioLabel(item.scenario))} · ${esc(caseStatus(item))}</span>`).join("");
    renderPhaseRail();
  }
  ...
  function actionMarkup(c){
    ...
    if(c.phase==="COMPLETE")return'<button class="secondary" data-sim-action="inspect">查看完整证据</button><button class="secondary" data-sim-action="replay">重新回放</button>';
    return"";
  }
  ...
  function renderDialog(){
    ...
    if(el("sim-dialog-phases"))el("sim-dialog-phases").innerHTML=PHASES.map(([_,label],i)=>`<span class="${ord[stage]===i?"current":""} ${ord[stage]>i||c?.phase==="COMPLETE"?"done":""}">${i+1}. ${label}</span>`).join("");
  ...
  function ensureStepDialog(){
    let d=el("sim-step-dialog");
    if(d)return d;
    d=document.createElement("dialog");
    d.id="sim-step-dialog";
    d.className="sim-step-dialog";
    d.innerHTML='<div class="sim-step-dialog-shell"><div class="sim-step-dialog-head"><div><span id="sim-step-kicker">步骤结果</span><h3 id="sim-step-title">处理完成</h3></div><button type="button" class="sim-step-dialog-close" data-step-close aria-label="关闭">×</button></div><div class="sim-step-dialog-body"><div id="sim-step-grid" class="sim-step-result-grid"></div><p id="sim-step-note" class="sim-step-result-note"></p></div><div id="sim-step-actions" class="sim-step-dialog-actions"></div></div>';
    el("sim-lab")?.appendChild(d);
    d.addEventListener("click",e=>{
      if(e.target===d||e.target.closest("[data-step-close]")){closeStepDialog();return;}
      const btn=e.target.closest("[data-step-action]");
      if(!btn)return;
      const action=btn.dataset.stepAction;
      closeStepDialog();
      const c=activeCase();
      if(action==="inspect"){openDialog();return;}
      ({defend,verify,recover,audit,replay})[action]?.(c);
    });
    return d;
  }

  function closeStepDialog(){
    const d=el("sim-step-dialog");if(!d)return;
    if(typeof d.close==="function"&&d.open)d.close();else d.removeAttribute("open");
  }

  function stepResultSpec(c,kind){
    if(!c)return null;
    const r=c.row, recoveryNeeded=!isNotNeeded(r.recovery_status)||!isNotNeeded(r.recovery_source);
    if(kind==="decision")return{
      kicker:"AI 风险研判完成",title:`${scenarioLabel(c.scenario)} · 正式决策已解锁`,tone:text(r.P)==="P3"?"risk":"good",
      cells:[["风险 R",risk(r.R),"accent"],["AI 判定 A",value(r.A),""],["策略 P",value(r.P),"accent"]],
      note:"R / A / P 来自正式事件记录；此弹窗仅用于答辩过程引导，不重新计算安全决策。",
      action:"defend",actionLabel:`执行 ${value(r.P)}`
    };
    if(kind==="defense")return{
      kicker:"主动防御步骤完成",title:"遏制结果已记录",tone:text(r.containment)==="CONTAINED"?"good":"risk",
      cells:[["策略 P",value(r.P),""],["Containment",value(r.containment),"accent"],["执行边界","SIMULATED",""]],
      note:"Permission / Network 在当前竞赛系统中为 SIMULATED；这里展示正式实验中的遏制结果。",
      action:"verify",actionLabel:"继续：完整性验证"
    };
    if(kind==="integrity")return{
      kicker:"完整性验证完成",title:"医疗数据可信状态已确认",tone:text(r.integrity_state)==="NORMAL"?"good":"risk",
      cells:[["Integrity",value(r.integrity_state),"accent"],["数据等级",value(r.D),""],["Trap",value(r.Trap),""]],
      note:recoveryNeeded?"正式事件字段表明需要继续进入可信恢复。":"该正式事件无需恢复，可直接生成审计结论。",
      action:recoveryNeeded?"recover":"audit",actionLabel:recoveryNeeded?"继续：可信恢复":"继续：生成审计"
    };
    if(kind==="recovery")return{
      kicker:"可信恢复完成",title:"恢复结果已记录",tone:"good",
      cells:[["Recovery",value(r.recovery_status),"accent"],["恢复来源",value(r.recovery_source),""],["Integrity",value(r.integrity_state),""]],
      note:"恢复状态和来源均读取正式事件字段；动画时长仅用于过程可视化。",
      action:"audit",actionLabel:"继续：生成审计"
    };
    if(kind==="complete")return{
      kicker:"事件闭环完成",title:"正式结果与证据已解锁",tone:"good",
      cells:[["Event Pool",value(r.event_pool_decision),"accent"],["Pipeline",num(r.total_pipeline_ms)!==null?`${Number(r.total_pipeline_ms).toFixed(2)} ms`:"--",""] ,["最终状态","COMPLETE",""]],
      note:"事件已经完成风险研判、策略处置、完整性/恢复与审计闭环。可以打开完整 EVENT STREAM 查看证据。",
      action:"inspect",actionLabel:"查看完整事件证据"
    };
    return null;
  }

  function openStepResult(c,kind){
    if(sim.mode!=="single"||!c||c.id!==sim.activeId)return;
    const spec=stepResultSpec(c,kind);if(!spec)return;
    const d=ensureStepDialog();if(!d)return;
    d.dataset.tone=spec.tone||"";
    el("sim-step-kicker").textContent=spec.kicker;
    el("sim-step-title").textContent=spec.title;
    el("sim-step-grid").innerHTML=spec.cells.map(([k,v,cls])=>`<div><span>${esc(k)}</span><strong class="${esc(cls||"")}">${esc(v)}</strong></div>`).join("");
    el("sim-step-note").textContent=spec.note;
    el("sim-step-actions").innerHTML=`<button type="button" class="secondary" data-step-close>稍后处理</button><button type="button" class="primary" data-step-action="${esc(spec.action)}">${esc(spec.actionLabel)}</button>`;
  ...
  function startSimulation(){
    ...
    const decision=c=>{c.phase="DECISION_READY";log("决策",`${c.row.event_id}: R=${risk(c.row.R)}, A=${value(c.row.A)}, P=${value(c.row.P)}`,text(c.row.P)==="P3"?"alert":"");render();openStepResult(c,"decision");};
    const t=timeline();
  ...
  function defend(c){
    ...
    const finish=()=>{c.phase="DEFENSE_DONE";log("防御",`${c.row.event_id}: containment=${value(c.row.containment)}`,text(c.row.containment)==="CONTAINED"?"good":"alert");render();openStepResult(c,"defense");};
  ...
  function verify(c){
    ...
    const finish=()=>{c.phase="INTEGRITY_DONE";log("完整性",`${c.row.event_id}: integrity_state=${value(c.row.integrity_state)}`,text(c.row.integrity_state)==="NORMAL"?"good":"alert");render();openStepResult(c,"integrity");};
  ...
  function recover(c){
    ...
    const finish=()=>{c.phase="RECOVERY_DONE";log("恢复",`${c.row.event_id}: recovery_status=${value(c.row.recovery_status)}`,"good");render();openStepResult(c,"recovery");};
  ...
  function audit(c){
    ...
    const finish=()=>{c.phase="COMPLETE";log("审计",`${c.row.event_id}: pool=${value(c.row.event_pool_decision)}, pipeline=${num(c.row.total_pipeline_ms)!==null?Number(c.row.total_pipeline_ms).toFixed(2)+" ms":"--"}`,"good");render();openStepResult(c,"complete");};
    const t=timeline();if(t)t.call(finish,[],.55);else setTimeout(finish,550);
  }
  function replay(c){if(!c)return;c.phase="ATTACKING";log("系统",`${c.row.event_id}: 重新回放。`);render();const a=()=>{c.phase="ANALYZING";render();};const d=()=>{c.phase="DECISION_READY";render();};const t=timeline();if(t){t.call(a,[],.55).call(d,[],1.15);}else{setTimeout(a,550);setTimeout(d,1150);}}
  function inspect(){openDialog();}

  function reset(){closeStepDialog();cancelTimelines();sim.started=false;sim.cases=[];sim.activeId=null;sim.logs=[];render();}
  ...
  function handleClick(e){
    ...
    const action=e.target.closest("[data-sim-action]");if(action){const c=activeCase();({defend,recover,verify,audit,replay,inspect})[action.dataset.simAction]?.(c);return;}
    ...
    if(e.target.closest("#sim-open-stream")||e.target.closest("[data-open-stream]")||e.target.closest("#simv2-stream-body")){openDialog();return;}
    if(e.target.closest("#sim-close-stream")){closeDialog();return;}
  }

  function loadFormalRow(rowIndex){
    const row=state.rows?.[rowIndex];if(!row)return;cancelTimelines();sim.mode="single";sim.selected=[row.scenario];sim.started=true;const c=makeCase(row.scenario,row);if(!c)return;c.phase="ATTACKING";sim.cases=[c];sim.activeId=c.id;sim.logs=[];document.querySelectorAll("[data-sim-mode]").forEach(b=>{const on=b.dataset.simMode==="single";b.classList.toggle("active",on);b.setAttribute("aria-selected",String(on));});log("攻击",`从正式事件样本载入 ${row.event_id}。`,"alert");render();const t=timeline();const a=()=>{c.phase="ANALYZING";render();};const d=()=>{c.phase="DECISION_READY";log("决策",`${c.row.event_id}: R=${risk(c.row.R)}, A=${value(c.row.A)}, P=${value(c.row.P)}`);render();openStepResult(c,"decision");};if(t){t.call(a,[],.55).call(d,[],1.15);}else{setTimeout(a,550);setTimeout(d,1150);}location.hash="#console";
  }

  function mount(){const root=el("sim-lab");if(!root||root.dataset.simMounted==="1")return;root.dataset.simMounted="1";ensureStepDialog();root.addEventListener("click",handleClick);el("sim-stream-dialog")?.addEventListener("click",e=>{if(e.target===e.currentTarget)closeDialog();});render();}

  window.MedShieldSimulationLabV2={mount,render,clearLog,reset,start:startSimulation,loadFormalRow,version:VERSION};
