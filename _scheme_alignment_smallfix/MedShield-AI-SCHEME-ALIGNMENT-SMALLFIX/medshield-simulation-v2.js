(function(){
  ...
  const VERSION = "V2-CORE-CENTRAL-STAGE";
  ...
  function stageText(c){
    ...
    if(c.phase==="ANALYZING")return["AutoEncoder → LightGBM 正在研判","U / F / N 先形成 X，AutoEncoder 计算 E_AE，LightGBM 再形成正式 R / A","等待 AI 输出 R / A"];
    if(c.phase==="DECISION_READY")return["AI 风险研判完成","AI 输出 R / A；Policy Engine 综合 R、D、A、C、Trap 形成正式 P","执行推荐策略"];
    if(c.phase==="DEFENDING")return["主动防御正在执行","策略正在改变攻击路径","等待遏制结果"];
    if(c.phase==="DEFENSE_DONE")return["主动防御结果已记录",`Containment = ${value(c.row.containment)}`,"执行密码学 + 水印/内容指纹联合完整性验证"];
    if(c.phase==="VERIFYING")return["正在执行联合完整性验证","密码学认证完整性与水印/内容指纹证据共同参与判定","等待正式完整性结果"];
    if(c.phase==="INTEGRITY_DONE")return["联合完整性结果已记录",`Integrity = ${value(c.row.integrity_state)}`,(!isNotNeeded(c.row.recovery_status)||!isNotNeeded(c.row.recovery_source))?"确认异常：启动可信恢复":"数据正常：无需恢复，进入安全评估/审计"];
    ...
    if(c.phase==="AUDITING")return["正在生成安全评估与审计结论","读取正式 Event Pool decision 与 pipeline 字段；事件入池须经过确认/筛选","等待审计完成"];
  ...
  function renderSystem(){const [label,tone]=systemPhase(), holder=el("sim-system-label")?.parentElement;if(el("sim-system-label"))el("sim-system-label").textContent=label;if(holder){holder.classList.remove("warn","alert");if(tone)holder.classList.add(tone);}}
  ...
  function startSimulation(){
    ...
    const analyze=c=>{c.phase="ANALYZING";log("AI",`${c.row.event_id}: U/F/N → X → AutoEncoder(E_AE) → LightGBM；AI 仅形成 R/A。`);render();};
  ...
  function verify(c){
    if(!c||c.phase!=="DEFENSE_DONE")return;c.phase="VERIFYING";log("验证",`${c.row.event_id}: 开始读取正式联合完整性结果（密码学 + 水印/内容指纹）。`);render();
  ...
  function audit(c){
    if(!c||!["INTEGRITY_DONE","RECOVERY_DONE"].includes(c.phase))return;c.phase="AUDITING";log("审计",`${c.row.event_id}: 读取正式 Event Pool decision 与 pipeline；AI prediction 不等于训练标签。`);render();
  ...
  function toggleScenario(name){if(sim.started)reset();if(sim.mode==="single")sim.selected=[name];else if(sim.selected.includes(name)){if(sim.selected.length>1)sim.selected=sim.selected.filter(x=>x!==name);}else if(sim.selected.length<selectionLimit())sim.selected=[...sim.selected,name];render();}
  function setMode(mode){if(!["single","parallel"].includes(mode))return;reset();sim.mode=mode;sim.selected=mode==="single"?[sim.selected[0]||"IMAGE_LEAKAGE"]:["IMAGE_LEAKAGE","EMR_LEAKAGE","ACCOUNT_ABUSE"];document.querySelectorAll("[data-sim-mode]").forEach(b=>{const on=b.dataset.simMode===mode;b.classList.toggle("active",on);b.setAttribute("aria-selected",String(on));});render();}
