(function(){
  ...
  const VERSION = "V2-CORE-BATCH-COMBINED-EVIDENCE";
  ...
  function stageText(c){
    ...
    if(c.phase==="ANALYZING")return["AutoEncoder → E_AE → LightGBM 正在研判","U / F / N 正在进入正式风险感知链","等待正式 R / A 解锁；P 由 Policy Engine 决策"];
    if(c.phase==="DECISION_READY")return["AI 风险研判完成","正式 R / A 已读取，Policy Engine 已生成 P","执行当前策略"];
  ...
  function batchCardMarkup(c,index){
    ...
    return `<article class="sim-batch-card" data-batch-card-id="${esc(c.id)}">
      ...
      <div class="sim-batch-keyline">
        <div><span>R</span><b>${risk(c.row.R)}</b></div>
        <div><span>A</span><b>${esc(value(c.row.A))}</b></div>
        <div><span>P</span><b>${esc(value(c.row.P))}</b></div>
      </div>
      <div class="sim-batch-results">
        <div><span>Containment</span><strong class="${text(containment)==="CONTAINED"?"good":""}">${esc(containment)}</strong></div>
        <div><span>Integrity</span><strong class="${text(integrity)==="NORMAL"?"good":""}">${esc(integrity)}</strong></div>
        <div><span>Recovery</span><strong>${esc(recovery)}</strong></div>
        <div><span>Pipeline</span><strong>${esc(pipeline)}</strong></div>
      </div>
    </article>`;
  }
  ...
  function renderBatchDialog(){
    ...
    host.innerHTML=`<div class="sim-batch-toolbar"><div><strong>${cases.length} 个事件已完成处置</strong><span>每个事件独立显示正式 R / A / P 与处置结果；可单独关闭，也可一键关闭全部。</span></div><span class="sim-batch-count">${cases.length} / ${sim.cases.length}</span></div><div class="sim-batch-grid">${cases.map(batchCardMarkup).join("")}</div>`;
  }
  ...
  function audit(c){
    if(!c||!["INTEGRITY_DONE","RECOVERY_DONE"].includes(c.phase))return;c.phase="AUDITING";log("审计",`${c.row.event_id}: 读取事件确认 / Event Pool 与 pipeline 正式字段。`);render();
